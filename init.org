...
good using it when writing English prose. Since using this mode mainly requires you write very long lines, it would make reading the =diff= between versions really difficult. Also, it destroys code blocks in =org-mode=, so I decide to not automatically enable it from now on.
