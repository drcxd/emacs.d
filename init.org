...
#+begin_src elisp
  ...
  (use-package
   ...
   (("C-c s l" . logos-focus-mode)
    ...
    ("C-c s k" . logos-forward-page-dwim)
    ("C-c s j" . logos-backward-page-dwim)))
#+end_src
...
Sometimes you want to set a timer, or you simply want to practice the
[[Pomodo][pomodoro technique]]. tmr helps you set and manage timers. pomm provide
pomodoro functions.
...
#+begin_src elisp
  (use-package
   ...
   :defer t
   :bind
   (("C-c d" . evilnc-comment-or-uncomment-lines)
    ("C-c D" . evilnc-copy-and-comment-lines)))
#+end_src
...
#+begin_src elisp
  (use-package
   ...
   :hook
   (lsp-mode . my-syntax-highlighting)
   :bind
   (("C-c l l" . lsp)
    :map lsp-mode-map
...
There are multiple scheme implementations, the one I choose is the
[[https://call-cc.org/][chicken compiler]]. According to its [[https://wiki.call-cc.org/emacs#builtin-scheme-support][documents]], to use it with Emacs
built-in support is easy.

#+begin_src elisp
  (setq scheme-program-name "csi -:c")
#+end_src

**** TODO Use Scheme in org-mode

To execute scheme code in org-mode, I still have to check the
[[https://orgmode.org/worg/org-contrib/babel/languages/ob-doc-scheme.html][documents]] of ob-doc-scheme.
...
https://www.mail-archive.com/emacs-orgmode@gnu.org/msg142485.html
https://gitlab.com/emacs-geiser/chicken/-/issues/23

According to the two links above. It seems that we have to:

1. Do what the chicken [[https://wiki.call-cc.org/emacs#geiser][wiki]] has said.
2. Install org-mode 9.6 or higher version.

Then we can normally execute code using org-babel. However, on
Windows, =chicken-install= does not work properly and I still can't find
