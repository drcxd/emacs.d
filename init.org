...
#+begin_src emacs-lisp
  ...
  ;; Collapse minor mode lighters, introduced in Emacs 31
  (setq mode-line-collapse-minor-modes '(not meow-mode
                                             meow-motion-mode
                                             meow-normal-mode
                                             meow-keypad-mode
                                             meow-insert-mode
    ...
    (setq display-buffer-alist
          '(((or "\\*\\(grep\\|xref\\|find\\)\\*"
            ...
            ((or (major-mode . dired-mode)
             ...
             (side . left)
             (slot . 1)))))
  ;; NOTE: There is an option `scroll-error-top-botoom', if set to
  ;; non-nil, scrolling past the beginning or end of the buffer will
  ;; first place the cursor at the beginning or end before yielding a
  ;; message. However, this interferes with elfeed-show-mode. When
  ;; pressing SPACE when reaching the end of the buffer in
  ;; elfeed-show-mode, it would first place the cursor to the end of the
  ;; buffer. You have to press SPACE again to switch to next feed. This
  ;; is annoying and redundant. Also,enabling this option does not
  ;; provide any actual benefits.
...
#+begin_src emacs-lisp
  (use-package treesit
    :init
    (setopt treesit-enabled-modes '(csharp-ts-mode)))
#+end_src

** Scroll

I read about these configurations in this [[https://www.jamescherti.com/emacs-scrolling-better-performance-usability/][article]].

#+begin_src emacs-lisp
  (setq hscroll-margin 2
        hscroll-step 1

        ;; NOTE: This makes the point location stay the same after you
        ;; execute scroll up or scroll down, but I find this behavior
        ;; not practical.
  ...
  ;; NOTE: This enables a line-by-line scroll behavior, but I find I am
  ;; now more used to the default behavior
...
Part of the configurations are copied from James Cherti's [[https://www.jamescherti.com/emacs-eglot-performance/][post]].

#+begin_src emacs-lisp
  (setq my/clangd-arguments
        '("--completion-style=detailed"
          ...
          "--pch-storage=memory"))
  (setq my/no-lsp-indent-modes '(lua-mode lua-ts-mode))

  (use-package eglot
    :defer t
    :init
    (setq eglot-autoshutdown t
          eglot-sync-connet nil
          eglot-events-buffer-config '(:size 0 :format short)
          eglot-report-progress nil
          eglot-code-action-indications nil)
    (defun my/eglot-indent-region-advice (oldfun &rest args)
      (if (and (eglot-managed-p)
               (eglot-server-capable :documentRangeFormattingProvider)
               (> (length args) 2)
               (not (member major-mode my/no-lsp-indent-modes)))
          (eglot-format (nth 0 args) (nth 1 args))
        (apply oldfun args)))
    (defun my/eglot--signal-textDocument/didClose-advice (oldfun &rest args)
      (unless (buffer-base-buffer)
        (apply oldfun args)))
    :config
    (add-to-list 'eglot-ignored-server-capabilities :documentOnTypeFormattingProvider)
    (add-to-list 'eglot-ignored-server-capabilities :inlayHintProvider)
    (add-to-list 'eglot-server-programs `((c-mode c-ts-mode c++-mode c++-ts-mode objc-mode) . ,(cons "clangd" my/clangd-arguments)))
    (advice-add 'indent-region :around #'my/eglot-indent-region-advice)
    (advice-add 'eglot--signal-textDocument/didClose :around #'my/eglot--signal-textDocument/didClose-advice)
    :hook
    ;; This hook moves `eglot--semtok-font-lock' to the front of the
    ;; `font-lock-keywords' list, especially prior to the font lock
    ;; keyword provided by `hl-todo', so that the it becomes compatible
    ;; with it.
    (eglot-semantic-tokens-mode . (lambda ()
                                    (font-lock-remove-keywords nil '((eglot--semtok-font-lock)))
                                    (font-lock-add-keywords nil '((eglot--semtok-font-lock)))
                                    (font-lock-flush)))
    :bind
    (("C-c e a" . eglot-code-actions)
     ...
     ("C-c e c" . eglot-show-call-hierarchy)
...
#+begin_src emacs-lisp
  (setenv "XMAKE_COLORTERM" "nocolor")
#+end_src
...
#+begin_src emacs-lisp
  ...
  (define-minor-mode my/visual-unfill-writing-mode
    "Toggle visual unfill writing mode.

  When enable the mode, it enables `visual-line-mode' and disable
  `auto-fill-mode'. When disable the mode, it disables `visual-line-mode'
  and enables `auto-fill-mode'."
    :lighter " VU"
    (if my/visual-unfill-writing-mode
        (progn
          (visual-line-mode 1)
          (auto-fill-mode -1))
      (visual-line-mode -1)
  ...
  (defun my/os-dark-theme-p ()
    "Returns non-if the OS is using a dark theme.

  If we can not determine OS theme, then return nil."
    (cond
     ((eq system-type 'windows-nt)
      (when-let* ((exe (or (executable-find "powershell.exe")
                           (executable-find "pwsh.exe"))))
        (with-temp-buffer
          (shell-command (format "%s -NoProfile -Command Get-ItemPropertyValue -Path HKCU:Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize -Name AppsUseLightTheme"
                                 exe)
                         (current-buffer))
          (eq 0 (string-match "0\n" (buffer-string))))))
     ((eq system-type 'gnu/linux)
      (let ((theme-config-file "~/.config/kdeglobals"))
        (when (file-exists-p theme-config-file)
          (when-let* ((os-theme (my/find-line-matching theme-config-file "LookAndFeelPackage")))
...
#+begin_src emacs-lisp
  (defvar my/svn-branch-matching-regexp
    (rx "^/branches/"
        (group (+? (or (syntax _) (syntax w))))
        "/")
    "Regular expression to match for branch name.")

  (defun my/svn-switch-branch-file (target file)
    "Switch branch to TARGET for FILE."
    (interactive "sBranch: \nfFile: ")
    (if (file-exists-p file)
        (with-temp-buffer
          (shell-command (format "svn info %s | grep \"Relative URL\"" file) (current-buffer))
          (let* ((output (string-trim (buffer-string)))
                 (relative-url (substring
                                output
                                (string-match-p "\\^" output)))
                 (at-trunk (string-match-p "\\^/trunk" relative-url))
                 source
                 target-relative-url)
            (if at-trunk
                (setq source "trunk")
              (unless (string-match my/svn-branch-matching-regexp
                                    relative-url)
                (user-error "Matching for branch name failed for %s"
                            relative-url))
              (setq source (match-string 1 relative-url)))
            (if (equal source target)
                (user-error "Switching for buffer %s from %s to %s. Ignored!" (buffer-name buffer) source target)
              (setq target-relative-url (if at-trunk
                                            (replace-regexp-in-string
                                             "\\^/trunk"
                                             (format "^/branches/%s" target)
                                             relative-url)
                                          (replace-regexp-in-string
                                           my/svn-branch-matching-regexp
                                           (if (equal target "trunk")
                                               (format "^/trunk/")
                                             (format "^/branches/%s/" target))
                                           relative-url)))
              (shell-command (format "svn switch \"%s\" \"%s\""
                                     target-relative-url
                                     file)))))
      (user-error "File not exist: %s" file)))

  (defun my/svn-switch-branch (target &optional buffer)
    "Switch branch to TARGET for the BUFFER file.

  If BUFFER is nil, then default to the current buffer."
    (interactive "sBranch: ")
    (if-let* ((file-name (buffer-file-name buffer))
              (file (expand-file-name file-name)))
        (my/svn-switch-branch-file file)
      (user-error "Can not locate file for buffer %s" buffer)))

  (defun my/svn-switch-branch-ibuffer (target)
    "Switch branch to TARGET for marked buffers."
    (interactive "sBranch: ")
    (require 'ibuffer)
    (dolist (buffer (ibuffer-get-marked-buffers))
      (my/svn-switch-branch target buffer)))

  (defun my/svn-switch-branch-vc-marked-files (target)
    "Switch branch to TARGET for marked files in vc-dir."
    (interactive "sBranch: ")
    (dolist (file (vc-dir-marked-files))
      (my/svn-switch-branch-file target file)))

  ;; Advise `vc-checkin' so that this function is called instead
  (defun my/tortoise-svn-commit-vc-marked-files ()
    "Invoke TortoiseProc to commit marked files in vc-dir."
    (interactive)
    (unless (executable-find "TortoiseProc")
      (user-error "TortoiseProc is not in PATH!"))
    (when-let* ((files (vc-dir-marked-files)))
      (let ((command "TortoiseProc /command:commit /path:"))
        (async-shell-command
         (concat command
                 (format "\"%s\"" (mapconcat #'identity files "*")))))))

  (defun my/tortoise-svn-commit (&optional dir)
    "Invoke TortoiseProc to commit files in the current directory."
    (interactive)
    (unless (executable-find "TortoiseProc")
      (user-error "TortoiseProc is not in PATH!"))
    (setq dir (if (and current-prefix-arg (null dir))
                  (read-directory-name "Commit in: ")
                "."))
    (async-shell-command
     (format "TortoiseProc /command:commit /path:\"%s\"" dir)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package standard-themes :ensure t :after modus-themes)
  (use-package ef-themes :ensure t :after modus-themes)

  (defun my/select-modus-theme-using-os-theme (&optional random-p)
    "Select a proper modus theme depends on the os theme."
    (interactive)
    (if (my/os-dark-theme-p)
        (if random-p
...
This package place the minibuffer completion interface to the center
of the screen, emulating the behavior of some modern editors.

The package does not work in WSL for now.
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    (defvar-local my/consult-ripgrep-default-dir nil
      "If non-nil, change the behavior of consult-ripgrep.

  If set to the symbol `ask', `consult-ripgrep' will prompts for the
  searching directory.  If set to a directory, `consult-ripgrep' will use
  it as the searching directory.  Otherwise, `consult-ripgrep' keeps the
  original behavior.")
    (put 'my/consult-ripgrep-default-dir 'safe-local-variable (lambda (_) t))

    (defun my/consult-ripgrep-advice (oldfun &rest args)
      (if (or current-prefix-arg
              (null my/consult-ripgrep-default-dir))
          (apply oldfun args)
        (pcase my/consult-ripgrep-default-dir
          ('ask (apply oldfun '(dummy))) ;; This makes consult-ripgrep asks for directories
          ((pred file-directory-p) (apply oldfun (list my/consult-ripgrep-default-dir)))
          (_ (apply oldfun args)))))
    :config
    ...
    (with-eval-after-load 'project
      (add-to-list 'project-switch-commands
                   '(consult-project-buffer "Consult buffer" "u"))
      (add-to-list 'project-switch-commands
                   '(consult-fd "Consult fd" "i")))
    :bind
...
Temple has a severe flaw: It does not support consecutive fields. I
can not accept this. Yasnippet handle this properly, so I decide to
switch back to yasnippet.
...
#+begin_src emacs-lisp
  (use-package org :defer t
    ...
    (when (featurep 'org)
      (unload-feature 'org))
    :vc (:url "https://code.tecosaur.net/tec/org-mode.git/"
              :branch "dev")
    :init
    (defvar my/org-babel-languages (list '(C . t)
                                         ...
                                         '(emacs-lisp . t)
                                         '(dot . t)))
    (defun org-rename-buffer (&optional buffer)
      "Try rename the current buffer if it is a org-mode buffer with a title."
      (when (eq major-mode 'org-mode)
        (save-excursion
          (org-set-regexps-and-options t) ;; update `org-file-tags'
    ...
    (define-minor-mode org-rename-buffer-mode
      "Minor mode that renames org mode buffer if it has a title."
      :global t
      (if org-rename-buffer-mode
          (progn
            (add-hook 'find-file-hook #'org-rename-buffer)
            (add-hook 'after-save-hook #'org-rename-buffer))
        (remove-hook 'find-file-hook #'org-rename-buffer)
        (remove-hook 'after-save-hook #'org-rename-buffer)))
    :hook
...
An interesting package that allows you to hand-draw images in org
mode. May be useful later.
...
#+begin_src emacs-lisp
  (use-package context-clues
    :ensure t
    :defer t
    :vc (:url "https://github.com/drcxd/context-clues"
              :branch "upgrade-to-emacs-31"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package flycheck
    ...
    :ensure t
    ;; NOTE: flycheck by default display errors through eldoc, but it
    ;; sometimes open a dedicated eldoc buffer, which feels
    ;; awkward. Thus I decide to no longer use it with eglot for now.
...
#+begin_src emacs-lisp
  (use-package diff-hl
    ...
    :hook
    (prog-mode . diff-hl-margin-local-mode))

  (use-package diff-hl-margin
    :after diff-hl
    :config
    (when-let* ((record (assoc 'change diff-hl-margin-symbols-alist)))
      (setcdr record "=")))
...
Note that git-gutter does not work well with eglot. If eglot is
enabled, then whenever the cursor is moved, the visual indicator
produced by git-gutter is hidden.

git-gutter-fringe seems able to work with eglot. It also provides
different visual results.

Also enable git-gutter-mode in any buffer that has a version control
backend, instead of buffers whose major mode is derived from
prog-mode.
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-clangd
    :defer t
    :config
    (setq lsp-clients-clangd-args my/clangd-arguments))
...
#+begin_src emacs-lisp
  (use-package eldoc-box
    ...
    (defun my/eldoc-box-toggle ()
      "Convenient wrapper to show, focus, and unfocus eldoc box frame."
      (interactive)
      (cond ((and (eldoc-box--frame-visible-p)
                  (eq (selected-frame) eldoc-box--frame))
             (eldoc-box-unfocus-frame))
            (t (eldoc-box-help-at-point))))
    :bind
    (([remap eldoc] . my/eldoc-box-toggle)))
#+end_src
...
- [[https://github.com/isamert/ellm.el][ellm]] is an /agent/ for Emacs, written in Emacs Lisp. It has no
  further dependencies. I would not try it now since it is in a really
  nascent stage.
- [[https://github.com/jamie-cui/magent][Magent]] another Emacs Lisp agent package.
...
#+begin_src emacs-lisp
  ...
  (use-package typst-ts-mode :defer t
    ...
    :ensure t
    :vc (:url "https://codeberg.org/YTG123/typst-ts-mode.git"
              :branch "fix-autoload")
    :init
...
#+begin_src emacs-lisp
  ...
  (use-package vulpea-journal
    ...
    (("C-c v j" . vulpea-journal)
     ...
     ("C-c v n" . vulpea-journal-next)
     ("C-c v p" . vulpea-journal-previous))
    :config
...
#+begin_example
  gcc -I. -O2 -Wall -Wextra -fPIC -shared -o jinx-mod.dll jinx-mod.c -I/mingw64/include/enchant-2 -L/usr/local/lib -lenchant-2
#+end_example
...
This package requires a canvas patch to work. See the GitHub page for
more details.

#+begin_src emacs-lisp
  (use-package doom-on-emacs
    :vc (:url "https://github.com/minad/doom-on-emacs.git")
    :defer t
    :ensure t)
#+end_src
...
#+begin_src emacs-lisp
  (use-package meow
    ...
    (defun my/meow-use-f5-exclusively ()
      "Make SPC no longer enable KEYPAD and serve as literal prefix."
      (define-key meow-normal-state-keymap (kbd "SPC") nil)
      (setq meow-keypad-literal-prefix 'f5))
