...
The following functions should be invoked in the custom file or interactively.
...
#+begin_src emacs-lisp
  ...
  (defun my/setup-fixed-pitch-font (font height weight)
  ...
  (defun my/setup-variable-pitch-font (font)
  ...
  fonts given."
    ...
    (my/setup-fixed-pitch-font fixed-pitch-font height fixed-pitch-weight)
    (my/setup-variable-pitch-font variable-pitch-font))

  (defun my/setup-chinese-font (font)
  ...
  (defvar my/current-font-profile "")
  (defun my/use-font (profile-name)
    "Apply the font profile specified by the given profile name."
    (interactive
     (let ((completion-ignore-case t))
       (list (completing-read "Font profile name: " (my/get-font-profile-names)))))
    (let ((profile (my/get-font-profile profile-name)))
      (if profile
          (progn
            (let ((fixed-pitch-font (my/font-profile-fixed-pitch-font profile))
                  (weight (my/font-profile-weight profile))
                  (variable-pitch-font (my/font-profile-variable-pitch-font profile)))
              (my/setup-fonts fixed-pitch-font my/font-size 'regular variable-pitch-font))
            (setq my/current-font-profile profile-name))
        (warn "No such font profile: MY/USE-FONT:" profile-name))))

  (defun my/use-font-by-num (n)
    (let ((len (length my/font-profiles)))
      (if (= 0 len)
          (message "Empty font profiles!")
        (let* ((idx (% n len))
               (profile-name (car (nth idx my/font-profiles))))
          (my/use-font profile-name)))))

  (defun my/use-font-by-day ()
    "Select a font profile depends on the date."
    (interactive)
    (let ((day (/ (time-convert (current-time) 'integer) (* 60 60 24))))
      (my/use-font-by-num day)))

  (defun my/use-font-rand ()
    "Select a font randomly."
    (interactive)
    (let ((n (random)))
      (my/use-font-by-num n)))

  (defvar my/launch-counter 0)

  (use-package savehist
    :init
    (setq savehist-save-minibuffer-history nil)
    :config
    (push 'my/launch-counter savehist-additional-variables))

  (defun my/use-font-by-launch ()
    "Select a different font each time Emacs is launched."
    (savehist-mode)
    (my/use-font-by-num my/launch-counter)
    (setq my/launch-counter (+ my/launch-counter 1))
    (savehist-save)
    (savehist-mode -1))

  (defun my/describe-current-font-profile ()
    "Describe the font profile currently used."
    (interactive)
    (message "The current font profile is %s" my/current-font-profile))

  ;; Deprecated code that is used to define functions that apply font
  ;; profiles. Use `my/use-font' now.

  ;; (defmacro my/def-use-font-function (name fixed-pitch-font variable-pitch-font)
  ;;   "Templates that is used to define various font using functions."
  ;;   `(defun ,(intern (concat "my/use-font-" name)) ()
  ;;      (interactive)
  ;;      (my/setup-fonts ,fixed-pitch-font 180 'regular ,variable-pitch-font)))

  (defvar my/font-profiles '())

  (defun my/add-font-profile (name fixed-pitch-font weight variable-pitch-font)
    (push (cons name (list fixed-pitch-font weight variable-pitch-font))
          my/font-profiles))

  (defun my/get-font-profile-names ()
    (mapcar #'car my/font-profiles))

  (defun my/get-font-profile (name)
    (assoc name my/font-profiles))

  (defun my/font-profile-fixed-pitch-font (profile)
    (nth 1 profile))

  (defun my/font-profile-weight (profile)
    (nth 2 profile))

  (defun my/font-profile-variable-pitch-font (profile)
    (nth 3 profile))
#+end_src
