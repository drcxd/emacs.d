...
#+begin_src emacs-lisp
  ...
  (defun my/unfill-writing-mode ()
    "Turn on `visual-line-mode' and turn off `auto-fill-mode'."
    (interactive)
    (visual-line-mode 1)
    (auto-fill-mode -1))

  (defun my/fill-writing-mode ()
    "Turn off `visual-line-mode' and turn on `auto-fill-mode'."
    (interactive)
    (visual-line-mode -1)
    (auto-fill-mode 1))
#+end_src
