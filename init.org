    ...
    (when-let ((e (meow--event-key last-input-event))
      ...
      (let ((has-sub-meta (meow--keypad-has-sub-meta-keymap-p)))
        (cond
         ...
         (meow--keypad-keys
          (push (cons 'control key) meow--keypad-keys)) ;; C- prefix
...
If we exchange the two =push= evaluation, then we change KEYPAD's
behavior to not automatically add =C-= prefix, but only when =SPC= is
pressed. This modification introduces a new problem: which-key prompt
for KEYPAD key bindings does not update to fit this change. I'll
