...
I use scheme mainly to learn SICP. SIPC has some customized code that
are frequently used in multiple examples. There is a Racket package
...
To use the code in the package inside Emacs, input the following in
the Racket session:

#+begin_src example
  ,enter "/path/to/package.rst"
#+end_src

The prompt of the Racket session should change from =racket@>= to
=racket@package.rst>=. Afterwards, the code defined in the package can
be used in the session and in org-mode. Note that to use the code
imported to the session, the org-mode code block must define the
=:session= header.
...
#+begin_src emacs-lisp
  (use-package geiser-racket
    :defer t
    :config
    ;; this forces geiser to use racket to evaluate code in org-mode
    ;; buffer
