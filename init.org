...
#+begin_src emacs-lisp
      ...
      (if profile
          (progn
            ...
            (let ((fixed-pitch-font (nth 1 profile))
                  (size (nth 2 profile))
                  (weight (nth 3 profile))
                  (variable-pitch-font (nth 4 profile)))
  ...
  (defun my/use-font-by-launch ()
    (let ((file-name (concat user-emacs-directory "launch")))
      (if (not (file-exists-p file-name))
          (write-region "('launch 0)" nil file-name 0))
      (with-temp-buffer
        (insert-file-contents file-name)
        (let ((launch (cadr (read (current-buffer)))))
          (write-region (format "('launch %d)" (+ 1 launch)) nil file-name 0)
          (my/use-font-by-num launch)))))
  ...
  (defvar my/font-profiles
    '(("IBM Plex" . ("IBM Plex Mono" 180 'regular "IBM Plex Sans"))
      ("Iosevka" . ("Iosevka" 180 'regular "Iosevka Aile"))
      ("Iosevka Comfy" . ("Iosevka Comfy" 180 'regular "Iosevka Comfy Duo"))
      ("JetBrains Mono" . ("JetBrains Mono" 180 'regular "IBM Plex Sans"))
      ("Victor Mono" . ("Victor Mono" 180 'regular "IBM Plex Sans"))))
#+end_src
