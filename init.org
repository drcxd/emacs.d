...
*** Set =find= on Windows :dependency:
...
figure out the location of the curosr, I can use =C-l=. Better, I can
...
#+begin_src emacs-lisp
  (defun prog-mode-setup ()
    ...
    ;; (electric-pair-mode t) ;; replace with smartparens
    (hs-minor-mode 1)
    (hl-line-mode 1))
...
I learned about this from this [[https://emacsredux.com/blog/2026/04/07/stealing-from-the-best-emacs-configs/][post]] in Emacs Redux.
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c O") 'blink-matching-open)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package modus-themes
    ...
    :defer t
    :init
    (global-set-key (kbd "C-c t g") 'modus-themes-toggle)
...
This package does not prove to be really useful in practice, so I
comment it out.
...
#+begin_src emacs-lisp
  ...
  ;;   :ensure `(:type file :main ,(file-name-concat elpaca-sources-directory "vertico" "extensions" "vertico-quick.el"))
  ;;   :after vertico
  ;;   :config
...
The built-in package hi-lock can serve this purpose as well.
...
Using vterm on Windows is difficult for now. First, it depends on an
external program [[https://github.com/xhcoding/conpty-proxy][conpty-proxy]]. Second, it requires compile a shared
library in MSYS UCRT shell. Yes, the library does not compile in a
MinGW shell. See this [[https://github.com/akermu/emacs-libvterm/pull/777][pull request]] for more detail. Only to make
things worse, vterm's does not perform well when running certain
application. My use case is to run Gemini-CLI in vterm, but every time
the vterm window resizes, Emacs just stops responding. As a result, I
decide to use a separate terminal program, at least on Windows, for
now.
...
#+begin_src emacs-lisp
  (if (not (eq system-type 'windows-nt))
      (use-package vterm :defer t
        :ensure t))
    ;; (use-package vterm :defer t
    ;;   ;; :ensure (:type git :host github :repo "xhcoding/emacs-libvterm")
    ;;   :ensure (:type git :host github :repo "kiennq/emacs-libvterm")
    ;;   :init
...
This package only works on Linux out-of-the-box. On Windows, it can
not properly pass =json= objects arguments to =math-preview=. Since I
have to use Emacs on Windows, I would try to look for another solution
that works across platform.

#+begin_src emacs-lisp
  (use-package math-preview :defer t
    :ensure t)
#+end_src
...
*** COMMENT Multimedia -- ready-player :dependency:

ready-player is a package that allows you to open multimedia files in
Emacs and play them just as you open any ordinary text file. Though
emms is the default package for multimedia on Emacs, it does not work
on Windows. See this [[https://www.eduardorobles.com/posts/streaming-music-in-emacs/][post]] for another one who struggled to make it
work on Windows. I personally can not make emms work on Windows for my
case -- playing local multimedia files. Currently, I do not use Emacs
to play albums, so ready-player is exactly what I need.
