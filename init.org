...
scheme mainly to learn SICP. SICP has some customized code that are
frequently used in multiple examples. There is a Racket package
...
choice than Chez when learning SICP. Instead of writing =scheme= code
block, writing =racket= code block take advantages of the =sicp= package.

The org-babel support for racket is done in the package
[[https://github.com/hasu/emacs-ob-racket][eamcs-ob-racket]] , which is only available on github. I have no other
choice but use =git submodule= to link it to my configuration
repository.
...
Currently, I would use Chez for pure Scheme programming and use Racket
for SICP related Scheme programming. Thus, I need not install the
=geiser-racket= package.
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'load-path (concat user-emacs-directory "site-lisp/emacs-ob-racket"))
  (add-to-list 'my/org-babel-languages '(racket . t))
#+end_src
