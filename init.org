...
#+begin_src emacs-lisp
  (use-package occult
    ...
    (defun my/toggle-hiding ()
      "Helper function that automatically call `occult-toggle' if region is
  active. Otherwise, call `hs-toggle-hiding'."
      (interactive)
      (if (region-active-p)
          (occult-toggle)
        (if (occult--overlay-at-point)
            (occult-toggle)
          (hs-toggle-hiding)))))
#+end_src
  ...
  (use-package meow
    ...
    (defun meow-setup ()
      ...
      (meow-normal-define-key
       ...
       '("{" . backward-paragraph)
       '("}" . forward-paragraph)
       `("/" . ,(if (functionp 'my/toggle-hiding)
                    'my/toggle-hiding
                  'hs-toggle-hiding))
       '("|" . occult-reveal-all)
       '("_" . occult-edit-region))
